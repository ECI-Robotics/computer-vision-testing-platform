...
class OpticalFlowCalculator:
    ...
    def processFrame(self, frame, distance=None, timestep=1):
        ...
        if self.prev_gray != None:
            ...
            flow = cv2.calcOpticalFlowFarneback(self.prev_gray, gray, pyr_scale=0.5, levels=1, winsize=13, iterations=20, poly_n=5, poly_sigma=1.1, flags=0)
            ...
            for y in range(0, flow.shape[0], self.move_step):
                ...
                """print('FlowSum_{4}: LEFT X:{0} Y:{1} - RIGHT X:{2} Y:{3}'.format(leftHalfXFlowSum,leftHalfYFlowSum,rightHalfXFlowSum,rightHalfYFlowSum,self.itCount));

                print(leftHalfXFlowSum+leftHalfYFlowSum, "flujo en izq")

                print(rightHalfXFlowSum+rightHalfYFlowSum, "flujo en der")"""
       ...
       # Normalize and divide by timestep
        return  leftHalfXFlowSum+leftHalfYFlowSum, rightHalfXFlowSum+rightHalfYFlowSum, frame2
